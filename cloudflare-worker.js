  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (provider === 'openai') {
        ...
        if (!response.ok) {
          return new Response(JSON.stringify({ error: { message: oData.error?.message || 'OpenAI error' } }), {
        ...
        return new Response(JSON.stringify(claudeFormat), { headers: corsHeaders });
      }
      ...
      if (provider === 'gemini') {
        const geminiBody = {
          contents: [],
          generationConfig: {
            maxOutputTokens: Math.min(body.max_tokens || 2000, 4000)
          }
        };
        ...
        if (!response.ok) {
          return new Response(JSON.stringify({ error: { message: gData.error?.message || 'Gemini error' } }), {
            status: response.status, headers: corsHeaders
          });
        }
